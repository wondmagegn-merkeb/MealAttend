...
export function UserForm({ onSubmit, initialData, isLoading = false, submitButtonText = "Submit", isProfileEditMode = false }: UserFormProps) {
  ...
  const form = useForm<z.infer<typeof currentSchema>>({
    ...
    defaultValues: initialData ? {
      ...initialData,
      role: initialData.role === 'Super Admin' ? 'Admin' : initialData.role, // Default to Admin if editing Super Admin
  ...
  useEffect(() => {
    if (initialData) {
      form.reset({
        ...initialData,
        role: initialData.role === 'Super Admin' ? 'Admin' : initialData.role,
        departmentId: initialData.departmentId || "",
        status: initialData.status || "Active",
        profileImageURL: initialData.profileImageURL || "",
      });
              ...
              <>
                <FormField control={form.control} name="role" render={({ field }) => (
                  <FormItem>
                    ...
                    <Select onValueChange={(field as any).onChange} value={(field as any).value} defaultValue={(field as any).value} disabled={currentUser?.role !== 'Super Admin' || initialData?.role === 'Super Admin'}>
                    ...
                    <FormDescription>The user's role in the system. Super Admins assign Admins. Admins assign Users. Super Admin role cannot be assigned or changed.</FormDescription><FormMessage />
                  </FormItem>
                )} />
